...
class TestRouting(unittest.TestCase):
    ...
    def test_escalation_and_cost(self):
        routed = RoutedTydex(
            [
    ...
    def test_per_call_min_confidence_override(self):
        routed = RoutedTydex([Tier(Tydex(make_weak()), threshold=0.4, label="weak")])
        r = routed.choice({}, ["a", "b"], min_confidence=0.9)
        self.assertEqual(r.escalations, ["weak"])
        self.assertEqual(r.tier, "weak")

    def test_human_fallback(self):
        ...
        with self.assertRaises(RequiresHuman) as ctx:
            routed.noul({}, "statement")
        self.assertEqual(ctx.exception.escalations, ["weak"])
        self.assertIn("state", ctx.exception.request)

    def test_human_tier_requires_request_context(self):
        with self.assertRaises(RequiresHuman) as ctx:
            RoutedTydex([Tier(action="human")]).choice({"x": 1}, ["a", "b"])

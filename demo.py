...
async def run_with(tdex: Tydex, label: str) -> None:
    print(f"=== {label} ===")
    ...
    s = await tdex.score(state, ["critical", "high", "medium", "low"], question="How urgent is this ticket?")
    print(f"score       -> {s.score}  (conf {s.confidence:.2f})")

    n = await tdex.noul(state, "This ticket must be handled within the SLA window.")
    print(f"noul        -> P(true) = {n.probability:.3f}  (bool: {n.bool_value}, conf {n.confidence:.2f})")
    print()


async def demo_calibration() -> None:
    samples = [
    ...
    result = await calibrate_temperature(Tydex(model), samples)
...
async def demo_escalation() -> None:
    ...
    r = await routed.choice({"ticket": "#9"}, ["refund", "replace"], question="Action?")
    print(f"choice  -> {r.result.choice}  conf {r.confidence:.2f}  via {r.tier}  (route {r.escalations}, cost {r.total_cost:.2f})")
    m = await routed.score({"ticket": "#9"}, ["urgent", "normal"], question="Urgency?")
    ...
    try:
        await human_routed.noul({"ticket": "#9"}, "Refund the customer.")
    except RequiresHuman as exc:
        print(f"noul    -> {exc}  (request keys: {sorted(exc.request)})")


async def demo_calibration_system() -> None:
    ...
    r = await calibrated.choice(state, ["refund", "replace", "wait"], question="Best action?")
    print(f"  applied -> choice {r.choice}, conf {r.confidence:.2f} ({r.source})")
    system.save()


async def main() -> None:
    await run_with(Tydex(MockBackend()), "Mock backend (no API key needed)")
    await demo_calibration()
    await demo_escalation()
    await demo_calibration_system()

    try:
        await run_with(Tydex(OllamaBackend(model="llama3.1:8b")), "Ollama local")
